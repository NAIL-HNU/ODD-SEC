...
int main(int argc, char** argv) {
    const std::string engineFile = "../panorama/model/yolox_temporal_v7_x86.engine";
    const std::string tenFolder  = "/home/zhx/dataset/Added/result/";
    const std::string outputFolder = "./output";

    // 修改为竖向模型的输入尺寸
    const int   INPUT_W = 480, INPUT_H = 640;  // 交换宽高，变为竖向
    const int   INPUT_C_SINGLE = 3;  // 第一个输入通道数
    const int   INPUT_C_TEN = 10;    // 第二个输入通道数
    ...
    // 获取Tensor信息 (TensorRT 8.x方式)
    // const int numInputs = engine->getNbInputs();
    // const int numOutputs = engine->getNbOutputs();
    // std::cout << "Input tensors: " << numInputs << ", Output tensors: " << numOutputs << "\n";
    ...
    // 设置输入尺寸 - 修复TensorRT API错误
    context->setInputShape(input1Name, Dims4(1, INPUT_C_SINGLE, INPUT_H, INPUT_W));
    context->setInputShape(input2Name, Dims4(1, INPUT_C_TEN, INPUT_H, INPUT_W));  // 改为batch=1
    ...
    for (const auto& entry : fs::directory_iterator(tenFolder)) {
        if (entry.is_regular_file() && entry.path().extension() == ".bin") {
            ...
            // 检查推理是否成功
            if (tInfer < 0.1f) {  // 如果推理时间异常短，可能失败了
            ...
            // 添加尺寸信息到图像上
            // cv::putText(vis, "Size: " + std::to_string(vis.cols) + "x" + std::to_string(vis.rows), 
            ...
            // 保存推理结果到输出文件夹 - 删除重复声明
            // fs::path inputPath = entry.path(); // 删除这行，因为上面已经声明过了
            fs::path outputPath = fs::path(outputFolder) / entry.path().filename();
            outputPath.replace_extension(".jpg");  // 将.bin换成.jpg
